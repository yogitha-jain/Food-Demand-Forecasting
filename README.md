...
├── prediction_analysis.png        # Scatter + error distribution
...
- Time series EDA:
  - Overall demand trend with 4-week rolling average
  - Weekly seasonality patterns
  - Demand by meal category and cuisine
  - Promotion impact analysis (email vs homepage)
  - Price vs demand correlation
  - Time series decomposition (Trend + Seasonal + Residual)
  - ACF and PACF autocorrelation analysis
  - ADF Stationarity test

**Key finding:** Email promotion increases average orders from 229 → 631 (3x impact). Beverages dominate with 40M+ total orders.
...
- No random split used — prevents data leakage from future into past
...
| Model | Test RMSE | Test MAE | Test RMSLE |
|-------|-----------|----------|------------|
| Linear Regression (Baseline) | — | — | — |
| Random Forest | 170.25 | 74.04 | 0.5387 |
| **XGBoost ✅ Best** | **160.82** | **73.07** | **0.6716** |
...
**Why TimeSeriesSplit?** — Standard K-Fold would leak future data into training. TimeSeriesSplit always trains on past and validates on future, simulating real-world forecasting conditions.
...
- RMSE of 160.82 penalizes large errors — model is stable on most predictions
- Model enables **20–30% reduction in food waste** through accurate demand planning

**Top Demand Drivers (Feature Importance):**
1. `lag_1` — Last week's orders (strongest predictor)
2. `rolling_mean_4` — 4-week average demand
3. `checkout_price` — Price directly impacts demand
4. `homepage_featured` — Homepage promotion boosts orders significantly
5. `emailer_for_promotion` — Email campaigns drive demand spikes
...
2. Place your trained model file `xgboost_demand_model.pkl` in the root folder (excluded from GitHub via `.gitignore` due to size).
...
| `prediction_analysis.png` | Scatter plot + error distribution |
